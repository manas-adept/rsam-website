...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  function openConfirmationModal(payload) {
    ...
    confirmSummaryBody.innerHTML = `
      ...
      <div class="confirm-grid">
        ...
        <div class="confirm-fee-breakdown" style="grid-column: span 2; background: ${BASE_FEE === 0 ? 'rgba(52, 211, 153, 0.08)' : 'rgba(31, 41, 55, 0.6)'}; border: 1px solid ${BASE_FEE === 0 ? 'rgba(52, 211, 153, 0.25)' : 'rgba(75, 85, 99, 0.4)'}; border-radius: 10px; padding: 1rem; margin-top: 0.5rem;">
          ${BASE_FEE === 0 ? `
            ...
            <div style="display: flex; flex-direction: column; gap: 0.6rem;">
              <label style="display: flex; align-items: center; gap: 0.6rem; background: rgba(16, 185, 129, 0.1); padding: 0.65rem 0.85rem; border-radius: 8px; cursor: pointer; border: 1.5px solid #10b981;">
                ...
                <div>
                  ...
                  <div style="font-size: 0.8rem; color: #d1d5db;">0% Gateway Fee · Pay <strong>₹${BASE_FEE.toFixed(2)}</strong> Total</div>
                </div>
              </label>
              <label style="display: flex; align-items: center; gap: 0.6rem; background: rgba(245, 158, 11, 0.1); padding: 0.65rem 0.85rem; border-radius: 8px; cursor: pointer; border: 1.5px solid rgba(245, 158, 11, 0.4);">
                ...
                <div>
                  ...
                  <div style="font-size: 0.8rem; color: #d1d5db;">+ 2% Fee &amp; GST · Pay <strong>₹${TOTAL_AMOUNT.toFixed(2)}</strong> Total</div>
                </div>
              </label>
            </div>
          `}
    ...
    confirmProceedBtn.onclick = () => {
      ...
      if (payMethod === 'upi_qr') {
        launchUpiQrCheckout(payload, BASE_FEE, (updPayload) => {
          submitEventRegistration(updPayload);
        }, () => {
  ...
  // Helper for Direct UPI QR Payment Gateway Modal
